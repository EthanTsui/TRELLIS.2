...
CHAMPION_CONFIG = {
    ...
    'schedule_rho': 7.0,
    'schedule_power': 2.0,
    # Triangular guidance schedule for texture (validated +0.6 vs beta(3,3), +1.1 vs constant)
    'tex_guidance_schedule': 'triangular',
    'resolution': '1024',
    'seed': 42,
    'decimation_target': 800000,  # matches app.py Gradio default
    'texture_size': 2048,
}
...
def generate_and_evaluate(pipeline, image_path, config, evaluator, envmap,
    ...
    if staged_bon > 1:
        ...
        outputs, latents = pipeline._run_staged_bon(
            ...
            tex_slat_sampler_params={
                ...
                "cfg_pp_steps": config.get('tex_cfg_pp_steps', config.get('cfg_pp_steps', 0)),
                "rectified_cfgpp": config.get('rectified_cfgpp', False),
                "rcfgpp_lambda_max": config.get('tex_rcfgpp_lambda_max', config.get('rcfgpp_lambda_max', 4.5)),
                "rcfgpp_gamma": config.get('tex_rcfgpp_gamma', config.get('rcfgpp_gamma', 0.0)),
                "rcfgpp_sigma_noise": config.get('tex_rcfgpp_sigma_noise', config.get('rcfgpp_sigma_noise', 0.005)),
                "multistep": config.get('multistep', False),
                ...
                "fdg_time_schedule": config.get('tex_fdg_time_schedule', config.get('fdg_time_schedule', 'fixed')),
            },
            pipeline_type={
    ...
    try:
        ...
        glb_kwargs = dict(
            ...
            remesh=config.get('remesh', True),
            remesh_band=config.get('remesh_band', 1),
